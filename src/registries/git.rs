...
pub fn download_repository(
    ...
    if !path.exists() {
        // Clone the project.
        let registry_cache_path_str = match path.to_str() {
            Some(data) => data,
            None => return Err("Failed to get registry cache path".to_string()),
        };

        // Display message.
        print!("Cloning repository {uri} into {registry_cache_path_str} ... ");
...
pub fn clone_and_checkout(
    ...
    // Clone the repository if it doesn't yet exists.
    if !path.exists() {
        match builder.clone(&uri, &path) {
            Ok(_) => (),
            Err(err) => {
